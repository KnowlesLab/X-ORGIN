...
get_psi <- function (ni, nj, fi, fj,
        ...
        tbl <- tbl[! to.exclude, ]
 				...
 				rowPsi <- function(row) (row[1]/row[2]-row[3]/row[4])*row[5]
 				return( sum(apply(tbl,1,rowPsi))/sum(tbl[,5]))
 			}else{
        to.exclude <- tbl$fi == 0 | tbl$fj == 0 | 
            tbl$ni < n | tbl$nj < n
...
#make pop from snapp format coordinates
make_pops_snapp <- function( coords, n=2 ){
    get_pops_w_multiple_samples <- function(coords,cutoff=3){
        #this function returns all locations (in lat, long) that have more than
        #cutoff samples. I'll use these as locations for analysis
        #the return is the id (i.e. col in data, row in coords) of the populations
...
find_origin <- function(pop_coords, all_psi, 
                        ...
                        xlen=50,  ylen=50, doPlot=F, doPdf=F,f.dist=f.dist, rescale = rescale,bbox = bbox,
                        IBRmap = NA,
             ...){
    ...
    res <- model_countries(data = data_countries[[1]],
               bbox = bbox,
                           ...
                           xlen=xlen,
                           ylen=ylen, f.dist=f.dist, IBRmap = IBRmap, ...)


    orig <- find_orig_table(bbox, res, xlen, ylen,rescale = rescale)
...
####add points to check for origins based on the resistance map instead of current maps#
model_countries <- function(data, bbox,  pop_coords,
                countries, pct=0.01,
            xlen=100, ylen=100, f.dist = "haversine", IBRmap = NA...){
    ...
    for( i in 1:length(s1)){
        for( j in 1:length(s2) ){
            ...
            if( ! is.na(coords2country(data.frame(x,y),f.dist = f.dist, IBRmap=IBRmap))){
                mdl <- model_1d( xy=c(x,y), data=data, pct=pct,f.dist=f.dist,IBRmap = IBRmap)
                d0[i, j] <- mdl$f_e
                rsq[i, j] <- mdl$rsq
                mdlq[[i]][[j]] <- mdl
            }
...
#------------------------------------------------------------
find_orig_table <- function(bbox, res, xlen, ylen,rescale){
    ...
    r1 = 1/(1+2*rescale*q)
    r2 = 1/(1+2*rescale*10*q)
    r3 = 1/(1+2*rescale*100*q)
    r <- 0.99
    d1pc <- (1-r)/(2*q*r)

    res.data <- ( c( coords_orig, q*rescale, r1, r2, r3, d1pc/rescale, smry$adj.r.squared,
      smry$coefficients[2,4] ) )
...
plot_map <- function( bbox, res, xlen=100, ylen=100){
    ...
    plot(NA, xlim=bbox[1,], ylim=bbox[2,], xlab="longitude", ylab="latitude")
    #plot(NA, xlim=c(-165,-125), ylim=c(55,71), xlab="longitude", ylab="latitude")
    ...
    #plot(m, xlim=c(-165,-125), ylim=c(55,71), add =T)
    #rel[is.na(rel)] <- N
    e <<- e
...
# or whether points in the resistance map is large enough to calculate 
# least cost distance
#------------------------------------------------------------
coords2country <- function(points,f.dist = "haversine", IBRmap = NA){
    if(f.dist == "resistance") {
      pointsSP = SpatialPoints(points, proj4string=CRS(proj4string(IBRmap[[1]])))
      val<- extract(IBRmap[[1]],pointsSP)
      if (is.na(val) | (val<= 1e-05)) {
        return(NA)
      }else{
        return(val)
      }
...
#------------------------------------------------------------
model_1d <- function(xy, data, pct=0.01, f.dist="haversine", IBRmap = NA){
    y = xy[2] 
    x = xy[1]
    #print(x)
    #print(y)
    ...
    if (f.dist=="euclidean"){
        ...
        d = f.dist(ixy, c(x,y)) - f.dist(jxy, c(x,y))             
    }else if(f.dist=="resistance"){ 
      ###add option to calculate resistance values among nodes 
        d = as.vector(costDistance(IBRmap[[2]],ixy,c(x,y))- costDistance(IBRmap[[2]],jxy,c(x,y)))
    }
...
run_region <- function( region, loc_file_id=out_file_id, 
                       xlen=100, ylen=100,f.dist,rescale,bbox,IBRmap = NA...){
    ...
    origin1 <- find_origin(pop_coords, all_psi,
                  region=region,
                   xlen=xlen,ylen=ylen,
                   doPlot=T, doPdf=pdf.name,f.dist=f.dist,rescale=rescale,bbox = bbox,IBRmap=IBRmap)
...
hets <- function(x)x * (1-x)*2
get_heterozygosity <- function( pop_data, pop_ss){
	colMeans(hets(pop_data/pop_ss),na.rm=T)
}

permute_data <- function(data, pops, n_permutations=1000,n ){
    ...
    for( i in 1:n_permutations){
        ...
        permuted_psi <- get_all_psi(per_data, per_ss,n=n)
...
test_ibd <- function(data, all_psi, pops, n_permutations=1000,n){
    permuted_data <- permute_data(data, pops, n_permutations,n)
...
#---------functions to read in resistance maps--------
readResistance<-function(resistanceMapName) {
  asc = read.asciigrid(resistanceMapName)
  r = raster(asc)
  s<-trim(r)
  #plot(s)
  fact <-ceiling(max(dim(s))/250)
  s1<-aggregate(s,fact=fact,method="bilinear")
  r<-s1
  r[is.na(r)]<-0.00001
  tr<-transition(r,mean,4)
  tr1<-geoCorrection(tr,type="r",multpl=FALSE)
  outmap<-list()
  outmap[[1]]<-r
  outmap[[2]]<-tr1
  return(outmap)
